...
def main():
    ...
    prefix='hlsp_k2superstamp_k2_photometer_c7-ruprecht147'
    ...
    bigarr = np.empty([408,408])
    bigarr[:] = np.nan
    bigarr = bigarr.astype('float32')

# Some images have a problem...
#    for i in range(0,4042):
##    for i in range(602,2614):
##    for i in range(2618,4042):
    for i in range(4042,4043):

# construct output primary extension
       outfile = prefix + '-bjd%.4f' % time[i] + suffix + '.fits'
       oldoutfile = 'Images_wcs/'+oldprefix + '_BJD%.4f' % time[i] + '_wcs.fits'
       print outfile
       print oldoutfile
...
# Add some history of file generation
       ...
       for file in filenames:
         hdu0.header.set('HISTORY',' '+file[39:])
